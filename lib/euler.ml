...
module Continued_fraction = struct
  (** Representation of numbers in the form of (sqrt(A) + B / D), whereas A, B
      and D are integers. *)
  module QZ = struct
...
let primitive_right_triangles p_max =
  ...
  |> Seq.concat_map begin fun m ->
      ...
      |> Seq.filter_map begin fun n ->
           ...
           *)
          if m mod 2 <> n mod 2 && gcd m n = 1 then
