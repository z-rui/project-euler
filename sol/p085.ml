...
let () =
  ...
  let test a b rect_cnt =
    let abs_err = Int.abs (rect_cnt - target) in
  ...
  for a = 1 to limit do
    ...
    let tb = b * (b + 1) / 2 in
    let rect_cnt = ta * tb in
    test a b rect_cnt;
    if rect_cnt < target then test a (b + 1) (rect_cnt + (ta * (b + 1)))
    else if rect_cnt > target then test a (b - 1) (rect_cnt - (ta * b))
  done;
  let a, b = !best in
  let ta = a * (a + 1) / 2 and tb = b * (b + 1) / 2 in
  Printf.printf "a=%d, b=%d, rect_cnt=%d, rectangle_count= %d * %d = %d\n" a b
    (a * b) ta tb (ta * tb)
